...
%BLENDIMAGEPAIR
    ...
    for c = 1:Cs
        ...
        elseif strcmp(mode, 'blend')
            ...
            % 
            binary_mask_s = ~binary_mask_s;
            binary_mask_s = bwdist(binary_mask_s,"euclidean");
            binary_mask_s = rescale(binary_mask_s, 0, 1);
            s = wrapped_imgs(:,:,c).*binary_mask_s;

            binary_mask_d = ~binary_mask_d;
            binary_mask_d = bwdist(binary_mask_d,"euclidean");
            binary_mask_d= rescale(binary_mask_d, 0, 1);
            d= wrapped_imgd(:,:,c).*binary_mask_d;

            channel_out = (s + d)./(binary_mask_s + binary_mask_d);
            channel_out(isnan(channel_out)) = 0;
