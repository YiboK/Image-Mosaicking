...
%               get you started.
    ...
    for n = 1:num_imgs
        ...
        [inliers_id, H_3x3] = runRANSAC(kp_n, kp_stitched, ransac_n, ransac_eps);

        dest_canvas_width_height = [size(stitched_img, 2), size(stitched_img, 1)]; 
        [mask, dest_img] = backwardWarpImg(varargin{n}, inv(H_3x3), dest_canvas_width_height);
        blended_result = blendImagePair(dest_img, mask, stitched_img, stitch_mask,'blend');
        
        stitched_img = blended_result;
        stitch_mask = stitched_img(:,:,1)>0;
        % ---------------------------------------
